...
def analyze_video_emotions(video_path, output_video_folder, output_data_folder):
    # Load the emotion detection model (FER2013)
    ...
    def detect_emotions(image):
        ...
        for (x, y, w, h) in faces:
            ...
            roi_gray = cv2.resize(roi_gray, (64, 64))  # Resize the image to (64, 64)
            roi_gray = roi_gray.astype("float") / 255.0
            roi_gray = np.expand_dims(roi_gray, axis=-1)  # Add a dimension for color channel
            roi_gray = np.expand_dims(roi_gray, axis=0)  # Add a dimension for batch
    ...
    # Create directories to save the modified video and emotion data files
    ...
    # Create a VideoWriter object to save the modified video in the output folder with the same file name
    ...
    # Read each frame from the video, detect emotions, and save the video with detected emotions
    frame_count = 0
    while cap.isOpened():
        ret, frame = cap.read()
        if ret:
            if frame_count % 10 == 0:  # Process every tenth frame to speed up the process
    ...
    print("Detected emotions, video saved, and data file created successfully.")

    return output_path  # Return the path to the resulting video file
